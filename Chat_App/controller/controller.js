...
//const {spawn} = require('child_process');           //extra added !!
    ...
    io.on('connection',function(socket){
        ...
        models.user.find({"handle" : handle},{friends:1,_id:0},function(err,doc){
            if(err){res.json(err);}
                ...
                list=[];//doc[0].friends.slice();          //added extra !! removed the friends to prevent error
                console.log(list);
        ...
        socket.on('group message',function(msg){
            console.log(msg);
			//extra added to run AI
			var dataToSend;

			const python = require('child_process').spawn('python', ['./controller/script2.py', msg.split("#*@")[0].toString()]);
